...
func TestServerMultiShardMoveWithCopy(t *testing.T) {
	...
	// log.Println("first update shard mapping")
	setup.UpdateShardMapping(map[int][]string{
		1:  {"n1", "n2"},
		2:  {"n1", "n2"},
		3:  {"n1", "n2"},
	...
	// log.Println("second update shard mapping")
	...
	for _, key := range keys {
		...
		// log.Println("Key should no longer exist")
		_, _, err := setup.NodeGet(keysToOriginalNode[key], key)
		assertShardNotAssigned(t, err)

		// log.Println("Key should exist on new node")
