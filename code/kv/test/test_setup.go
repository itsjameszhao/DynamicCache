...
// Creates only one replica per shard. Allocates shard replicas between nodes in a round-robin fashion.
func MakeManyNodesWithManyShardsSimple(numShards int, numNodes int) kv.ShardMapState {
	shardsToNodes := make(map[int][]string, numShards)
	nodeNames := make([]string, 0, numNodes)
	for i := 1; i <= numNodes; i++ {
		nodeNames = append(nodeNames, fmt.Sprintf("n%d", i))
	}
	for shard, idx := 1, 0; shard <= numShards; shard, idx = shard+1, idx+1 {
